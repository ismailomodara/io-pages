<template>
    <div ref="blockImage" class="block-image relative h-[250px] w-[100%] shadow-none bg-[#f3f4f5] rounded">
        <template v-if="content && content.id">
            ...
            <div class="block-image-change absolute -top-[14px] left-[5px] p-1 text-xs bg-black text-white cursor-pointer invisible " @click="$emit('action', { name: 'image-update'})">
        ...
        <div v-else class="h-[100%] w-[100%] flex flex-col justify-center items-center">
            ...
            <Button class="text-xs" size="small" type="secondary" @click="$emit('action', { name: 'image-update', value: '' })">Click to add image</Button>
        </div>
    </div>
</template>
